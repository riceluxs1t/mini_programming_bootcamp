...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        homework_object = Homework.objects.get(homework_name=homework_name)

        for module in homework_object.modules.split(','):
            s3_client.download_file(
                Bucket=S3_SUBMISSION_BUCKET_NAME,
                Key='homework/' + homework_name + '/' + user_name + '/' + module + ".py",
                Filename='grader/submissions/' + homework_name + '/' + user_name + '/' + module + ".py"
            )
